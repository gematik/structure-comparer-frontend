...
/**
 * Component that displays warnings about missing package dependencies.
 * Shows a collapsible warning banner when dependencies are missing or have version conflicts.
 */
@Component({
  ...
  styleUrls: ['./dependency-warning.component.css']
})
export class DependencyWarningComponent implements OnInit, OnChanges {
  ...
  /** Whether the warning panel is expanded */
  isExpanded = true;
  ...
  get hasIssues(): boolean {
    ...
    return this.missingDependencies.length > 0 || this.versionMismatches.length > 0;
  }
   ...
   */
  get versionMismatches(): VersionMismatch[] {
    return this.analysisResult?.version_mismatches || [];
  }
